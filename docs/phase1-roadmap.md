...
```solidity
constructor(
    address _admin,        // 管理者アドレス（multisig 推奨）
    address _jpyc,         // JPYC v2 ERC-20 アドレス
    address _feeRecipient  // DAO Gnosis Safe 2-of-3 アドレス
) {
...
> **注意**: ERC-2771 `trustedForwarder` は v2.2 のスコープ外。ガスレス送金は引き続き EIP-3009（JPYC の `transferWithAuthorization`）で実現する。

**fee 内部分配ヘルパー**:
```solidity
// jobId は内部 uint64（_keyToId[jobKey] で取得済み）、bytes32 jobKey ではない
...
**追加イベント**:
```solidity
// jobId は uint64（内部 ID）、jobKey（bytes32）ではない
...
- constructor の zero address リバート確認（admin, jpyc, feeRecipient 各引数; feeRecipient は `FeeRecipientZero` カスタムエラー）
- 総計 60+ テストでグリーン
...
取引完了（confirmDelivery / claimExpired）
         ...
         └── DAO Gnosis Safe 2-of-3: reward × 0.1%
                    │
                    ├── signer 1: maintainer（またろ氏）  ← 候補: 0x66F1274aD5d042b7571C2EfA943370dbcd3459aB（Phase 1 開始時に正式確定）
                    ├── signer 2: community rep A     ← 候補: 0x46B073fA24489f7fd7aDE773d0459e04F5934BaF（Phase 1 開始時に正式確定）
                    └── signer 3: community rep B     ← 候補: 0xFCeA8435dCbBA7f3b1DA01E8EA3f4aF234a20bcb（Phase 1 開始時に正式確定）
                              ↓
