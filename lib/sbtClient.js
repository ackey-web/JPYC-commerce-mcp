 ...
 * @returns {{ rank: string, autoApproveLimit: number, scoreInt: number }}
 */
export function computeRank(trustScore) {
  ...
  if (scoreInt >= 100) return { rank: 'Platinum', autoApproveLimit: PLATINUM, scoreInt };
  if (scoreInt >= 60)  return { rank: 'Gold',     autoApproveLimit: GOLD,     scoreInt };
  if (scoreInt >= 30)  return { rank: 'Silver',   autoApproveLimit: SILVER,   scoreInt };
  return                      { rank: 'Bronze',   autoApproveLimit: BRONZE,   scoreInt };
}
...
export function buildMetadataURI(trustScore, completionCount, issuedAt) {
  ...
  if (autoApproveLimit !== null && autoApproveLimit !== undefined) {
