 ...
 *   claimExpired(bytes32)                     → 0xb16e1343
 *   cancelBounty(bytes32)                     → 0x3b0b43a6
 *   depositWithAuthorization(bytes32,uint128,uint256,uint256,bytes32,uint8,bytes32,bytes32)
 *                                             → 0xe3ee160e (EIP-3009: lib/relayerClient.js)
 */
...
const SEL = {
  ...
  claimExpired:     'b16e1343', // claimExpired(bytes32)
  cancelBounty:     '3b0b43a6', // cancelBounty(bytes32)
};
...
/**
 * claimExpired(bytes32 jobKey)
 * バウンティが期限切れの場合にクライアントが資金を回収する（OPEN → EXPIRED）。
 *
 * @param {string} jobKey バウンティの jobKey（bytes32 hex）
 */
export function buildClaimExpiredInstruction(jobKey) {
  return instruction({
    selector: SEL.claimExpired,
    args: [toBytes32(jobKey)],
    description: `BountyEscrow.claimExpired: jobKey=${jobKey}`,
    decodedArgs: { jobKey },
  });
}

/**
 * cancelBounty(bytes32 jobKey)
 * クライアント（poster）がOPEN状態のバウンティをキャンセルし、JPYC を回収する。
 * コントラクト側の要件: msg.sender == job.poster && status == OPEN
 *
 * @param {string} jobKey バウンティの jobKey（bytes32 hex）
 */
export function buildCancelBountyInstruction(jobKey) {
  return instruction({
    selector: SEL.cancelBounty,
    args: [toBytes32(jobKey)],
    description: `BountyEscrow.cancelBounty: jobKey=${jobKey}`,
    decodedArgs: { jobKey },
  });
}
